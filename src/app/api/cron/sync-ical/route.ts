...
export async function GET(request: NextRequest) {
  ...
  const { data: properties } = await db.from('properties').select('*')
  ...
  for (const prop of properties) {
    if (!prop.ical_url) { results.push({ name: prop.name, error: 'No iCal URL' }); continue }

    const icalText = await fetchIcal(prop.ical_url)
    if (!icalText) { results.push({ name: prop.name, error: 'Failed to fetch iCal' }); continue }
